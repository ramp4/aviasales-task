      ...
      {props.segments.map((item, index) => {
        ...
        return (
          <div key={index} className="segment">
            <p className="flight">
              <span className="flight__way">{`${props.segments[index].origin} - ${props.segments[index].destination}`}</span><br/>
              <span className="flight__time">{props.segments[index].date}</span>
            </p>
            <p className="duration">
              <span className="duration__title">В ПУТИ </span><br/>
              <span className="duration__time">
                {props.segments[index].duration}
              </span>
            </p>
            <p className="stops">
              <span className="stops__title">{`Количество пересадок: ${props.segments[index].stops.length}`}</span><br/>
